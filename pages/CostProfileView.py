...
from ui.CostProfileView_ui import UI_CostProfileView

class CostProfileView(QtWidgets.QWidget):

    def __init__(self, receiver_name, cost_type, session, parent=None):
        super(CostProfileView, self).__init__(parent)
        self.receiver_name = receiver_name
        self.cost_type = cost_type
        self.session = session
        self.ui =  UI_CostProfileView()  # Instantiate the UI class
        ...
        try:
            ...
            receivers = (
                ...
                .filter(and_(
                    ...
                    DealerModel.entry_name == self.cost_type
                 )
                )
            ).all()
            ...
            for receiver in receivers:
                ...
                }.get(receiver.entry_name, "অফিস খরচ")
                self.ui.tableWidget.insertRow(row)
                self.ui.tableWidget.setItem(row, 0, QtWidgets.QTableWidgetItem(str(receiver.id)))
                self.ui.tableWidget.setItem(row, 1, QtWidgets.QTableWidgetItem(cost_type))
                self.ui.tableWidget.setItem(row, 2, QtWidgets.QTableWidgetItem(str(receiver.name)))
                self.ui.tableWidget.setItem(row, 3, QtWidgets.QTableWidgetItem(str(receiver.date)))
                self.ui.tableWidget.setItem(row, 4, QtWidgets.QTableWidgetItem(str(receiver.paying_amount)))
                self.ui.tableWidget.setItem(row, 5, QtWidgets.QTableWidgetItem(str(receiver.entry_by)))
                row += 1
