...
from ui.loanView_ui import UI_loanView

class LoanProfileView(QtWidgets.QWidget):

    def __init__(self, payer_name, phone, session, parent=None):
        super(LoanProfileView, self).__init__(parent)
        self.payer_name = payer_name
        self.phone = phone
        self.session = session
        self.ui = UI_loanView()  # Instantiate the UI class
        self.ui.setupUi(self)  # Pass self to the setupUi method
        self.setup_ui()


    def setup_ui(self):
        self.setWindowTitle(f"{self.payer_name} - এর জন্য লেনদেন")
    ...
    def filter_data(self):
        try:
            ...
            row = 0
            for payer in payers:
                if payer.entry_name == "borrowing":
                    entry_name = "ঋণ গ্রহণ"
                else:
                    entry_name = "ঋণ পরিশোধ"
            ...
            QtWidgets.QMessageBox.critical(None, "Loan Profile View Error", f"An error occurred while filtering data: {e}")
    ...
    def openPrintMemo(self):
        ...
        for page in range(total_pages):
            try:
                ...
                self.ui_print_form.ui.name.setText(self.payer_name)
