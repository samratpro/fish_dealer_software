...
from ui.loanPayingView_ui import UI_loanPayingView

class LoanProfileView(QtWidgets.QWidget):

    def __init__(self, receiver_name, phone, session, parent=None):
        super(LoanProfileView, self).__init__(parent)
        self.receiver_name = receiver_name
        self.phone = phone
        self.session = session
        self.ui = UI_loanPayingView()  # Instantiate the UI class
        self.ui.setupUi(self)  # Pass self to the setupUi method
        self.setup_ui()


    def setup_ui(self):
        self.setWindowTitle(f"{self.receiver_name} - এর জন্য লেনদেন")
        ...
        seven_days_ago = self.today_date_raw - timedelta(days=30)
    ...
    def filter_data(self):
        try:
            ...
            payers = (
                ...
                .filter(
                    DealerModel.name == self.receiver_name,
                    and_(
                        DealerModel.entry_name.in_(["giving_loan", "receiving_loan"]),
            ...
            row = 0
            for payer in payers:
                if payer.entry_name == "giving_loan":
                    entry_name = "ঋণ প্রদান"
                else:
                    entry_name = "প্রদেয় ঋণ গ্রহণ"
                self.ui.tableWidget.insertRow(row)
                self.ui.tableWidget.setItem(row, 0, QtWidgets.QTableWidgetItem(str(payer.id)))
                self.ui.tableWidget.setItem(row, 1, QtWidgets.QTableWidgetItem(entry_name))
                self.ui.tableWidget.setItem(row, 2, QtWidgets.QTableWidgetItem(str(payer.name)))
                self.ui.tableWidget.setItem(row, 3, QtWidgets.QTableWidgetItem(str(payer.date)))
                self.ui.tableWidget.setItem(row, 4, QtWidgets.QTableWidgetItem(str(payer.paying_amount)))
                self.ui.tableWidget.setItem(row, 5, QtWidgets.QTableWidgetItem(str(payer.receiving_amount)))
                self.ui.tableWidget.setItem(row, 6, QtWidgets.QTableWidgetItem(str(payer.entry_by)))
                row += 1
        except Exception as e:
            QtWidgets.QMessageBox.critical(None, "Loan Paying Profile View Error", f"An error occurred while filtering data: {e}")
